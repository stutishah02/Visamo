...
namespace WebApplication3.Controllers
{
    public class SecretaryController : Controller
    {
        ...
        public ActionResult NoticeCreate(Notice n)
        {
            ...
            {
                Users uu = _context.Users.Where(u => u.Rid == 7).ToList().First();
                n.Uid = uu.Uid;
                _context.Add(n);
        ...
        public ActionResult Maintenance()
        {
            ...
            var residents = _context.Users.Where(u => u.Rid == 9).ToList();
        ...
        public ActionResult ViewMaintenance(string? status, string? wing)
        {
            ...
            using (var db = new visamodbContext()) // Replace with your actual DbContext
            {
                ...
                else if (status == "Overdue")
                {
                    ...
                    maintenanceData = maintenanceData
                        .Where(i => i.Status == "Pending" && i.Date < new DateTime(today.Year, today.Month, 1))
                        .ToList();
                ...
                maintenanceData = maintenanceData
    .OrderByDescending(i => i.Id) // or use i.Date if you prefer sorting by date
    .ToList();
  ...
  .ToList());
        ...
        public ActionResult IncomeView()
        {
            ...
            using (var db = new visamodbContext())
            {
                var incomeData = db.Income
                    .Where(i =>
                        (i.Description.Contains("maintenance") && i.Status.ToLower() == "paid") // paid maintenance only
                        || (!i.Description.Contains("maintenance")) // non-maintenance (manual) income
                    )
                   .OrderByDescending(i => i.Id) // 👈 Show latest entries first
            .ToList();
   ...
   .ToList());
        ...
        public IActionResult Resolve(int id)
        {
            try
            {
                var complaint = _context.Complain.FirstOrDefault(c => c.Cid == id);
                if (complaint == null)
                {
                    return NotFound();
                }
                ...
                complaint.Rdate = DateTime.Now; // Set resolved date
                _context.Update(complaint); // Ensure EF tracks the change
                _context.SaveChanges();
